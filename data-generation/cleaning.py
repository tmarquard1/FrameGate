...
def load_mp4(file_path, image_size=(224, 224)):
    ...
    Args:
        file_path: path to the location of an mp4 file.
    ...
    # Convert the list of frames to a TF tensor
    video = tf.convert_to_tensor(frames, dtype=tf.float32)
    # Normalize the video to [0, 1]
    video = video / 255.0
    return video
...
initial_state = model.init_states(clapping[tf.newaxis, ...].shape)
